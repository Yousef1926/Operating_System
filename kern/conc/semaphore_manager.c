...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
    ...
    int ans = allocate_semaphore_object(&semaphore_ptr);
     if(ans != E_NO_SEMAPHORE)
     {
    	 semaphores[ans].empty = 0;
         semaphores[ans].ownerID = ownerEnvID ;
         strcpy(semaphores[ans].name, semaphoreName);
         semaphores[ans].value   = initialValue ;
         return ans;
     }
