...
void env_free(struct Env *e)
{
	...
	//if(MemBlockNodes != NULL)
	//{
		// LIST_REMOVE(&AvailableMemBlocksList,MemBlockNodes );

		 // LIST_INSERT_HEAD(&FreeMemBlocksList, MemBlockNodes);
	//}
	       // *MemBlockNodes=NULL;
	...
	// [8] free the page DIRECTORY from the main memory
	        for (uint32 i =USER_HEAP_START; i < USER_HEAP_MAX; i+=PAGE_SIZE)
	            {
	        	uint32 *ptr=NULL;
	        	struct FrameInfo * temp = get_frame_info (e->env_page_directory,
	        					       i, &ptr);
