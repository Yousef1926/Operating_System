...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	struct Share *cur = NULL ;
    int ret = allocate_share_object(&cur);
	if (ret != E_NO_SHARE)
	{
	   uint32 l = ROUNDDOWN((uint32) virtual_address,PAGE_SIZE);
	   uint32 r = ROUNDUP((uint32) virtual_address + size,PAGE_SIZE);
	   uint32 nums = (r - l) / PAGE_SIZE;
		for (uint32 i = 0; i < nums; i++)
		{
			uint32 curVa = l + (i * PAGE_SIZE);
			uint32 * ptr = NULL;
			struct FrameInfo * tmp = get_frame_info(curenv->env_page_directory,curVa,&ptr);
			allocate_frame(&tmp);
			map_frame(myenv->env_page_directory, tmp, curVa,
			PERM_WRITEABLE | PERM_USER);

			add_frame_to_storage(cur->framesStorage, tmp, i);

		}
		cur->ownerID = ownerID;
		strcpy(cur->name, shareName);
		cur->references = 1;
		cur->size = size;
		cur->empty = 0;
		cur->isWritable = isWritable;
		return ret;
	}
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address) {
	...
	if (idx != E_SHARED_MEM_NOT_EXISTS) {
		uint32 nums = ROUNDUP(shares[idx].size / PAGE_SIZE, 1);

		for (int i = 0; i < nums; i++) {
			uint32 curva = (uint32) virtual_address + (i * PAGE_SIZE);
			struct FrameInfo *tmp = get_frame_from_storage(
					shares[idx].framesStorage, i);
			if (shares[idx].isWritable == 1) {
				map_frame(myenv->env_page_directory, tmp, curva,
				PERM_USER | PERM_WRITEABLE);
			} else {
				map_frame(myenv->env_page_directory, tmp, curva, PERM_USER);
			}
		}
		shares[idx].references++;
		return idx;
	}
	return E_SHARED_MEM_NOT_EXISTS;
...
int freeSharedObject(int32 sharedObjectID, void *startVA) {
  ...
  //int ans = get_share_object_ID (shares[sharedObjectID].ownerID, shares[sharedObjectID].name);
  //if (ans == E_SHARED_MEM_NOT_EXISTS)
  //  return E_SHARED_MEM_NOT_EXISTS;
	int32 ans = sharedObjectID;
  struct Share *tmp = &shares[ans];
  int l = ROUNDDOWN((uint32 )startVA, PAGE_SIZE);
  int r = ROUNDUP((uint32 )startVA + tmp->size, PAGE_SIZE);
  uint32 nums = (r - l) / PAGE_SIZE;
  for (uint32 i = 0; i < nums; i++)
    {
      uint32 curVa = l + (i * PAGE_SIZE);
      uint32 * ptr = NULL;
      struct FrameInfo * temp = get_frame_info (myenv->env_page_directory,
					       curVa, &ptr);
      ...
      for (int j = 0; j < 1024; j++)
	{
      ...
      if (fail)
	{
	  pd_clear_page_dir_entry (myenv->env_page_directory, curVa);
	  kfree ((void*) ptr);

	}

    }
    //clear_frames_storage(tmp->framesStorage);
    tmp->references--;
    if(tmp->references == 0)
